...
func progress(dayProgess: Int) -> String{
    let msg: String
    let meta = 100
    if dayProgess == meta {
        msg = "Voce atingiu seu destino"
    }else{
        msg = "voce atingiu \(dayProgess)%"
    }
    return msg
}
...
for item in milesToWalk {
    let msg = progress(dayProgess: item)
    print(msg)
}
/*:

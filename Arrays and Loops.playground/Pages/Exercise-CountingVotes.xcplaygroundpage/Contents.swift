...
func calcVotes (votes: [Bool]){
    ...
    if votesYes > votesNo {
        print("Votos sim ganharam tiveram \(votesYes)")
    }else if votesNo == votesYes{
        print("Empate")
    }else{
        print("Votos nao ganharam tiveram \(votesNo)")
    }
}

calcVotes(votes: shouldMascotChangeVotes)
calcVotes(votes: shouldInstallCoffeeVendingMachineVotes)
calcVotes(votes: shouldHaveMorePollOptionsVotes)
/*:
### Extension:
